...
def generate_header_h(f, num_vectors, seed_start):
    f.write("#ifndef TESTVECTOR_MULTI_H\n")
    f.write("#define TESTVECTOR_MULTI_H\n\n")
    f.write("#include <stdint.h>\n\n")
    f.write(f"#define NUM_TEST_VECTORS {num_vectors}\n")
    f.write(f"// Generated by generate_multi_vectors.py\n")
    f.write(f"// Starting Seed: {seed_start}\n\n")
    f.write("// Declare that these variables exist in another object file\n")
    f.write(f"extern uint32_t message[{num_vectors}][32];\n".format(num_vectors=num_vectors))
    f.write(f"extern uint32_t K_X[{num_vectors}][32];\n".format(num_vectors=num_vectors))
    f.write(f"extern uint32_t K_Y[{num_vectors}][32];\n".format(num_vectors=num_vectors))
    f.write(f"extern uint32_t K_Z[{num_vectors}][32];\n".format(num_vectors=num_vectors))
    f.write(f"extern uint32_t s[{num_vectors}][32];\n".format(num_vectors=num_vectors))
    f.write(f"extern uint32_t Public_X[{num_vectors}][32];\n".format(num_vectors=num_vectors))
    f.write(f"extern uint32_t Public_Y[{num_vectors}][32];\n".format(num_vectors=num_vectors))
    f.write(f"extern uint32_t Public_Z[{num_vectors}][32];\n".format(num_vectors=num_vectors))
    f.write(f"extern uint32_t K_X_Modn[{num_vectors}][32];\n".format(num_vectors=num_vectors))

    f.write("\n// Start Point G \n")
    f.write("extern uint32_t G_X[32];\n")
    f.write("extern uint32_t G_Y[32];\n")
    f.write("extern uint32_t G_Z[32];\n\n")

    f.write("#endif // TESTVECTOR_MULTI_H\n")
...
def main():
    ...
    output_filename_c = "../sw_project/src/sw/tests/testvector_multi.c"
    output_filename_h = "../sw_project/src/sw/tests/testvector_multi.h"
    
    print(f"Generating {num_vectors} test vectors starting from seed '{start_seed}'...")
    print(f"Output file: {output_filename_c}")
    ...
    # 2. Write File
    with open(output_filename_c, 'w') as f:
    ...
    with open(output_filename_h, 'w') as f:
        generate_header_h(f, num_vectors, start_seed)
